...
namespace DeepSky.Haze
{
    ...
    public class DS_HazeZone : MonoBehaviour
    {
        ...
        private void Setup()
        {
            ...
            m_BlendRangeInverse = 1.0f / Mathf.Max((Mathf.Min(m_AABB.extents.x, m_AABB.extents.z, m_AABB.extents.y) * m_BlendRange), Mathf.Epsilon);
        }
        ...
        public float GetBlendWeight(Vector3 position)
        {
            ...
            float dZ = Mathf.Abs(m_AABB.extents.y - Mathf.Abs(localPosition.y));
            float dY = Mathf.Abs(m_AABB.extents.z - Mathf.Abs(localPosition.z));
